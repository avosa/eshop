...
const Categories = (props) => {
  ...
  const addCategory = async (event) => {
    ...
    setCategory({
      ...category,
      categories: [...category.categories, categoryName.categoryName],
    });
    let url = "";
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/categories/create-category?token=" +
        props.token;
    } else {
      url =
  ...
  const getCategories = async () => {
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/categories/product-categories?token=" +
        props.token;
    } else {
      url =
        process.env.REACT_APP_PRODUCTION_API_URL +
  ...
  const deleteCategory = async (event) => {
    let categoryToDelete = category.categories.filter(
      (category) => category === event.target.getAttribute("value")
    );
    let updatedList = category.categories.filter(
      (category) => category !== event.target.getAttribute("value")
    );
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/categories/delete-category?token=" +
        props.token;
    } else {
      url =
        process.env.REACT_APP_PRODUCTION_API_URL +
  ...
  const errorDeleting = () =>
    toast.warning("Error! Deleting Category!", {
      position: toast.POSITION.TOP_LEFT,
    });

  const errorAdding = () =>
    toast.warning("Error! Adding Category!", {
      position: toast.POSITION.TOP_LEFT,
    });
  ...
  return (
    <div>
      <Navbar />
      <Helmet>
        <title>Categories | Mobicenter | Kenyas Number One Mobile Shop</title>
      </Helmet>
