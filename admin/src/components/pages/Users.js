...
const Users = (props) => {
  ...
  const deleteUser = async (email, userRole) => {
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/admins-customers-delete?token=" +
        props.token;
    } else {
      url =
        process.env.REACT_APP_PRODUCTION_API_URL +
    ...
    let response = await fetch(url, {
      ...
      body: JSON.stringify({ email: email, userRole: userRole }),
    });
    let data = await response.json();
                          ...
                          <tr>
                            ...
                            <td>
                              <i
                                ...
                                onClick={() =>
                                  deleteUser(value.email, value.user_role)
                                }
                              ></i>
                            </td>
                          </tr>
