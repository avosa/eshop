...
const Dashboard = (props) => {
  ...
  const [totalWeeklyProfit, setTotalWeeklyProfit] = useState({
    totalWeeklyProfit: 0,
  });
  const [totalDailyProfit, setTotalDailyProfit] = useState({
    totalDailyProfit: [],
  });
  ...
  const getTotalWeeklySales = async () => {
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/total-weekly-sales?token=" +
        props.token;
    } else {
      url =
        process.env.REACT_APP_PRODUCTION_API_URL +
  ...
  const getTotalWeeklyProfit = async () => {
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/total-weekly-profit?token=" +
        props.token;
    } else {
      url =
        process.env.REACT_APP_PRODUCTION_API_URL +
    ...
    if (data) {
      setTotalWeeklyProfit({
        ...totalWeeklyProfit,
        totalWeeklyProfit: data.TotalWeeklyProfit,
      });
    }
  };
  ...
  const getTotalDailyProfit = async () => {
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        process.env.REACT_APP_DEVELOPMENT_API_URL +
        "/api/v1/total-daily-profit?token=" +
        props.token;
    } else {
      url =
        process.env.REACT_APP_PRODUCTION_API_URL +
    ...
    if (data) {
      setTotalDailyProfit({
        ...totalDailyProfit,
        totalDailyProfit: data.TotalDailyProfit,
      });
    }
  };
  ...
  return (
    <div>
      <Navbar />
      <Helmet>
        <title>Mobicenter Dashboard</title>
      </Helmet>
                      ...
                      <div className="row">
                        <div className="col-md-7">
                          ...
                          <h3>
                            ...
                            }).format(totalSales.total)}
                            .<span className="decimal-point">00</span>
                          </h3>
                          <h6>
                            {"+" +
                        ...
                        <div className="col-md-7">
                          ...
                          <h3>
                            {new Intl.NumberFormat("en-GM", {
                              ...
                              currency: "GMD",
                              maximumSignificantDigits: 12,
                            }).format(totalProfit.totalProfit)}
                            .<span className="decimal-point">00</span>
                          </h3>
                          <h6>
                            {"+" + totalWeeklyProfit.totalWeeklyProfit} This
                            Week
                          </h6>
                        </div>
