...
const AddUser = (props) => {
  ...
  const register = async (event) => {
    ...
    if (process.env.NODE_ENV === "development") {
      url =
        ...
        "/api/v1/create-admin-or-customer?token=" +
        props.token;
    } else {
      url =
  ...
  const emailExists = () =>
    toast.warn("Email Exists!", {
      position: toast.POSITION.TOP_LEFT,
    });

  const usernameExists = () =>
    toast.warn("Username Exists!", {
      position: toast.POSITION.TOP_LEFT,
    });
  ...
  return (
    <div>
      <Navbar />
      <Helmet title="Add User | Mobicenter | Kenyas Number One Mobile Shop" />
